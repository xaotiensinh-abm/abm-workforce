...
- Cần soạn hợp đồng mới → dùng `skywork-doc` (mặc định) hoặc `docx-official` (nếu user yêu cầu local)
...
## Related Skills
- compliance-checker, labor-law, ip-protection, skywork-doc (mặc định tạo HĐ), docx-official (local only)
