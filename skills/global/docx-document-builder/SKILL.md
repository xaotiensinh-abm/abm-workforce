...
Yêu cầu Agent chạy script Python đã được đóng gói sẵn trong thư mục `scripts/`:

```bash
# Antigravity Global Environment:
python ~/.gemini/antigravity/skills/docx-document-builder/scripts/build_docx.py "đường_dẫn_tới_file.md" "đường_dẫn_tới_file_đích.docx"
```

## Features Nổi Bật (Automated by Script)
