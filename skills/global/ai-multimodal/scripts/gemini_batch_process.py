...
Supports Gemini 3, 2.5, and 2.0 models.
"""
...
SUPPORTED_MODELS = {
    ...
    'gemini-2.5-flash-image': 'models/gemini-2.5-flash-preview-native-audio-dialog',
    # Gemini 2.0 Series
...
def get_model_name(model: str) -> str:
    ...
    # Otherwise return as-is (API will validate)
    return model
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Transcribe multiple audio files (Gemini 3)
  %(prog)s --files *.mp3 --task transcribe --model gemini-3-pro-preview

  # Transcribe multiple audio files (Gemini 2.5)
  ...
  # Generate images (Gemini 3 - Latest)
  %(prog)s --task generate --prompt "A mountain landscape" \\
    --model gemini-3-pro-image-preview --aspect-ratio 16:9

  # Generate images (Gemini 2.5 - Stable)
  %(prog)s --task generate --prompt "A mountain landscape" \\
    ...
    parser.add_argument('--model', default='gemini-2.5-flash',
                       help='Gemini model to use (default: gemini-2.5-flash). '
                            'Available: gemini-3-pro-preview, gemini-3-pro-image-preview, '
